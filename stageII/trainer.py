...
class CondGANTrainer(object):
    def __init__(self,
        ...
        self.model_path = cfg.TRAIN.PRETRAINED_STACKGAN_MODEL
    ...
    def prepare_trainer(self, discriminator_loss, generator_loss,
                        hr_discriminator_loss, hr_generator_loss):
        ft_lr_retio = cfg.TRAIN.FT_LR_RATIO
        ...
        self.ft_generator_trainer = \
            self.define_one_trainer(hr_generator_loss,
                                    self.generator_lr * cfg.TRAIN.FT_LR_RATIO,
                                    'g_')
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % cfg.GPU_ID):
                ...
                summary_writer = tf.train.FileWriter(self.log_dir,
                                                        sess.graph)
    ...
    def eval_one_dataset(self, sess, dataset, save_dir, subset='test'):
        count = 0
        print('num_examples:', dataset._num_examples)
        while count < dataset._num_examples:
            start = count % dataset._num_examples
            images, embeddings_batches, savenames, captions_batchs =\
                ...
                for j in range(numSamples):
                    hr_samples, samples =\
                        sess.run([self.hr_fake_images, self.fake_images],
                                 {self.embeddings: embeddings_batches[i]})
                    samples_batches.append(samples)
                    hr_samples_batches.append(hr_samples)
                self.save_super_images(images, samples_batches,
                                       hr_samples_batches,
                                       savenames, captions_batchs,
                                       i, save_dir, subset)
